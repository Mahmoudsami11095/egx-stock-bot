...
function fetchTradingViewScan(): Promise<StockResult[]> {
...
function calculateFairValue(stock: StockResult): { fairValue: number; confidence: 'HIGH' | 'LOW' } {
  const price = stock.currentPrice;
  const low52 = stock.fiftyTwoWeekLow || price * 0.7;
  const high52 = stock.fiftyTwoWeekHigh || price * 1.3;

  if (stock.eps && stock.eps > 0) {
    // Simple PE-based valuation (sector PE ~13.5 for EGX)
    const sectorPE = 13.5;
...
): { signalType: string; signalScore: number; reasons: string[] } {
