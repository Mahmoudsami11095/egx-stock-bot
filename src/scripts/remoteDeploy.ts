...
conn.on('ready', () => {
  ...
  const commands = [
    ...
    'echo azureuserSami@11095 | sudo -S bash -c "cd /root/egx-stock-bot && git clean -fd && git reset --hard HEAD && git pull && npm install --legacy-peer-deps && rm -rf dist && npm run build:backend && PORT=5000 pm2 start dist/index.js --name egx-stock-bot && pm2 save && pm2 status"'
  ].join(' && ');
